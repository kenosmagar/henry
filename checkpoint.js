...
function salarioAnual(empleados, nombre) {
  ...
  for(i = 0; i < empleados.length; i++){
    if(empleados[i].nombre == nombre){
      return salAnual = empleados[i].salario * 12
    }
  }
...
function index() {
  ...
  // Tu código aca:
  Array.prototype.encontraIndex = function(valIndex){
  ...
  class Empleado {
    ...
    getTareas() {
      ...
      for(i = 0; i<this.tareas.length; i++){
        lisTareas.push(this.tareas[i].tarea);
      }
      return lisTareas;
    }
    ...
    getTareasPrioritarias(prioridad) {
      ...
      // }]
      let listPrioridades = [];
      let emptareas = this.tareas;

      for(i = 0; i<emptareas.length; i++){
        if(emptareas[i].prioridad > prioridad){
          listPrioridades.push({
            nombre: emptareas[i].nombre,
            prioridad: emptareas[i].prioridad,
          });
        }
      }
...
function filtrar(funcion) {
  ...
  Array.prototype.filtrar = function(cb){
    ...
    for(i = 0; i < this.length; i++){
      if(cb(this[i])){
        newArray.push({
          price: this[i].price, name: this[i].name});
      }
    }
    return newArray;
  }
